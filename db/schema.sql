...
CREATE TABLE IF NOT EXISTS users (
    ...
    email VARCHAR(100) DEFAULT NULL UNIQUE COMMENT '邮箱（注册必填；找回密码的唯一凭据。列仍可空：存量账号有未绑定的，空邮箱存 NULL 以避免唯一约束冲突）',
    ...
    token_version INT NOT NULL DEFAULT 0 COMMENT '令牌代次：改密/找回/管理员重置时+1，使该用户此前签发的 JWT 全部失效（JWT 的 tv 声明与此比对，不等即拒）',
